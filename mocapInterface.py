...
class rigidBody(viz.EventClass):
	...
	def _createVizNode(self,shape,size):
		...
		if(shape == 'box' ):
			...
			self.vizNode = vizshape.addCylinder(height=size[0],radius=size[1], alpha = 0.3,color=viz.BLUE)
			self.vizNode.setAxisAngle(0,0,1,90)
	...
	def resetRigid( self, allMarkers_midx ):
		...
		if( self.fileName.find('ShutterGlass')> -1 ):
			...
			center_GlobalXYZ = [
					  (newPos_midx_GlobalXYZ[0][0] + newPos_midx_GlobalXYZ[1][0] + newPos_midx_GlobalXYZ[2][0] + newPos_midx_GlobalXYZ[4][0])/4,
					  (newPos_midx_GlobalXYZ[0][1] + newPos_midx_GlobalXYZ[1][1] + newPos_midx_GlobalXYZ[2][1] + newPos_midx_GlobalXYZ[4][1])/4,
					  (newPos_midx_GlobalXYZ[0][2] + newPos_midx_GlobalXYZ[1][2] + newPos_midx_GlobalXYZ[2][2] + newPos_midx_GlobalXYZ[4][2])/4]
					  
					  
		elif( self.fileName.find('nvis')> -1 ):
			print 'mocapInterface.rigidBody.resetRigid: For '+ self.fileName + ' using mean of first and last markers as COM'
...
class mocapInterface(viz.EventClass):
	...
	rigidBodyShapes_ridx,rigidBodySizes_ridx,rigidBodyToggleVisibility_ridx, phaseSpaceMode):
		...
		self.owlParamMarkerCondThresh = 500
		
		#Retrives the correct mode to use as set by the user
		profileMode = self.getProfileMode(phaseSpaceMode)
		
		# Init server
		if (owlInit(self.serverAddress,profileMode) < 0): 
		#if ((self.serverAddress,#OWL_POSTPROCESS) < 0): Adds a lot of lag!
	...
	def getProfileMode(self,phaseSpaceMode):
		return {
			1 : OWL_MODE1,
			2 : OWL_MODE2,
			3 : OWL_MODE3,
			4 : OWL_MODE4
		}.get(phaseSpaceMode, 1)
...
if __name__ == "__main__":
	...
	rigidFileNames_ridx = ['hmd_nviz.rb','Paddle_Left.rb','Paddle_Right.rb']
	...
	rigidBodySizes_ridx = [[.1],[.03,.09], [.03,.09]]
	
	# Is the rigid body visible on startup?
	rigidBodyToggleVisibility_ridx = [0,1,1]
	...
	vizact.onkeydown( 'o', mocap.resetRigid, 'paddle' );
